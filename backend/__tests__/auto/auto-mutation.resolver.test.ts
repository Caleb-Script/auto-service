...
describe('GraphQL Mutations', () => {
  ...
  test('Neues Auto mit ungültigen Daten', async () => {
    ...
    const expectedMsg = [expect.stringMatching(/^grundpreis /u)];
  ...
  test('Auto mit ID 1 mit ungültigen Daten aktualisieren', async () => {
    ...
    const expectedMsg = [expect.stringMatching(/^kilometerstand /u)];
  ...
  test('Nicht-Vorhandenes Auto(ID420) aktualisieren', async () => {
    ...
    expect(message).toBe(`Es gibt kein Auto mit der ID: ${id.toLowerCase()}.`);
